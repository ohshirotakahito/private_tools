...
def compute_baseline_threshold(y, threshold_k=THRESHOLD_K,
    ...
    if len(y_f) >= w:
        baseline = percentile_filter(y_f, percentile=baseline_percentile, size=w, mode="nearest")
    elif len(y_f) > 0:
...
class NpySignalPicker:
    ...
    def on_export_current(self, event):
        ...
        print(f"[SAVED] {self.out_csv} ({len(rows)} pulses from this file, k={self.threshold_k:.1f})")
