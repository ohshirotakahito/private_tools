...
def _scan_available_batches(seq_data_root):
    ...
    for sub in sorted(seq_data_root.iterdir(), reverse=True):
        ...
        try:
            ...
            seq = str(df.iloc[0].get("sequence", "?"))
            bc = str(df.iloc[0].get("selectBC", "?"))
            exp = df.iloc[0].get("experiment_name", "") if "experiment_name" in df.columns else ""
            exp = str(exp) if pd.notna(exp) and str(exp).strip() else ""
