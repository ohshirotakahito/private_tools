...
# 過去のバッチを見たいときだけ、ここにサブフォルダ名を指定する。
...
if BATCH_DIR_OVERRIDE:
    save_dir = _seq_data_root / BATCH_DIR_OVERRIDE
    if not save_dir.exists():
        raise FileNotFoundError(
            f"指定されたバッチフォルダが見つかりません: {save_dir}\n"
            "BATCH_DIR_OVERRIDE の値(フォルダ名)を確認してください。"
        )
else:
    _latest_pointer = _seq_data_root / "_latest_batch.txt"
    if _latest_pointer.exists():
        _latest_name = _latest_pointer.read_text(encoding="utf-8").strip()
        save_dir = _seq_data_root / _latest_name
        print(f"[info] 最新バッチを自動選択しました: {_latest_name}")
    else:
        # 後方互換: サブフォルダ分けする前の旧バージョンで生成したデータ
        # (seq_data/ 直下に manifest.csv がある場合)はそちらを使う。
        save_dir = _seq_data_root
