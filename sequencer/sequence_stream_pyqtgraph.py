...
class _QtCompat:
    _ENUM_GROUPS = (
        ...
        "MouseButton", "KeyboardModifier", "TransformationMode", "CursorShape",
    )
...
class _ClickableLabel(QtWidgets.QLabel):
    """クリックできるQLabel(ロゴマークをボタン代わりにするため)"""
...
class ReadmeDialog(QtWidgets.QDialog):
    """sequencerフォルダのREADME.mdを表示するダイアログ。
    日本語(README.md)と英語(README_en.md)をボタンで切り替えられる。
    英語版ファイルがまだ無い場合は、その旨を案内するメッセージを表示する。"""
    ...
    def __init__(self, parent):
        ...
        self.lang_jp_btn = QtWidgets.QPushButton("\u65e5\u672c\u8a9e")
        self.lang_en_btn = QtWidgets.QPushButton("English")
        header.addWidget(self.lang_jp_btn)
        header.addWidget(self.lang_en_btn)
        v.addLayout(header)

        self.text_view = QtWidgets.QTextEdit()
        self.text_view.setReadOnly(True)
        self.text_view.setStyleSheet(
            f"QTextEdit {{ background-color:{CARD_BG}; color:{TEXT_PRIMARY}; border:1px solid {CARD_BORDER}; "
            "border-radius:8px; font-family:'DejaVu Sans Mono',monospace; font-size:12px; padding:10px; }}"
        )
        v.addWidget(self.text_view, stretch=1)
        ...
        close_btn.setStyleSheet(
            ...
            "border-radius:6px; padding:8px 20px; font-family:'DejaVu Sans Mono',monospace; }}"
        )
        close_btn.clicked.connect(self.accept)
        close_row.addWidget(close_btn)
        v.addLayout(close_row)

        self.lang_jp_btn.clicked.connect(lambda: self._switch_lang("ja"))
        self.lang_en_btn.clicked.connect(lambda: self._switch_lang("en"))
        self._switch_lang("ja")

    def _style_lang_btn(self, btn, active):
        if active:
            btn.setStyleSheet(
                f"QPushButton {{ background-color:{ACCENT_BLUE}; color:#04202E; border:none; "
                "border-radius:6px; padding:6px 14px; font-weight:700; font-family:'DejaVu Sans Mono',monospace; }}"
            )
        else:
            btn.setStyleSheet(
                f"QPushButton {{ background-color:{CARD_BG}; color:{TEXT_PRIMARY}; border:1px solid {CARD_BORDER}; "
    ...
    def _switch_lang(self, lang):
        self._style_lang_btn(self.lang_jp_btn, lang == "ja")
        self._style_lang_btn(self.lang_en_btn, lang == "en")

        path = BASE_DIR / ("README.md" if lang == "ja" else "README_en.md")
        if path.exists():
            try:
                text = path.read_text(encoding="utf-8")
            except Exception as e:
                text = f"(README\u306e\u8aad\u307f\u8fbc\u307f\u4e2d\u306b\u30a8\u30e9\u30fc\u304c\u767a\u751f\u3057\u307e\u3057\u305f: {e})"
        elif lang == "ja":
            text = f"README.md \u304c\u898b\u3064\u304b\u308a\u307e\u305b\u3093\u3002\n\u63a2\u3057\u305f\u5834\u6240: {path}"
        else:
            text = (
                "README_en.md \u304c\u307e\u3060\u3042\u308a\u307e\u305b\u3093\u3002\n\n"
                "\u82f1\u8a9e\u7248READMEを用意するには、sequencerフォルダに\n"
                "README_en.md という名前でファイルを追加してください。\n\n"
                f"\u63a2\u3057\u305f\u5834\u6240: {path}"
            )
        self.text_view.setPlainText(text)


class AboutJinDialog(QtWidgets.QDialog):
    """JINマークをクリックしたときに表示する、マークとプロジェクト名の由来の説明"""

    def __init__(self, parent, logo_path=None):
        super().__init__(parent)
        self.setWindowTitle("About JIN")
        self.resize(560, 520)
        self.setStyleSheet(f"background-color:{PAGE_BG};")
        ...
        if logo_path is not None and Path(logo_path).exists():
            ...
            if not pix.isNull():
                ...
                logo_lbl.setPixmap(pix.scaledToWidth(72, _Qt.SmoothTransformation))
                logo_lbl.setAlignment(_Qt.AlignCenter)
        ...
        def add_heading(text):
            ...
            lbl.setStyleSheet(
                f"color:{ACCENT_BLUE}; font-size:14px; font-weight:700; "
                "font-family:'DejaVu Sans Mono',monospace; border:none;"
            )
            cv.addWidget(lbl)
        ...
        add_heading("JINマークについて / About the JIN Mark")
        add_body(
            "JINのマークは、中央のDNA二重らせんと、それを左右から読み取るセンサーを"
            "組み合わせたデザインです。分子がもつ微細な情報を先端計測技術で捉え、"
            "解析するというプロジェクトの理念を表しています。"
        )
        add_body(
            "The JIN mark combines a central DNA double helix with sensors reading it "
            "from both sides. It represents the project's philosophy of capturing and "
            "analyzing the minute information held within molecules using cutting-edge "
            "measurement technology."
        )

        add_divider()

        add_heading("プロジェクト名「JIN」の意味 / The Meaning of the Name \u201cJIN\u201d")
        add_body(
            "「JIN」は、江戸時代の和算で非常に小さな桁を表した「塵（じん）」に由来します。"
            "目に見えないほど微小な世界を象徴する言葉であり、分子という究極の単位を"
            "一つひとつ計測し、その情報を読み解くというプロジェクトの目標を表しています。"
        )
        add_body(
            "\u201cJIN\u201d derives from \u201c\u5875 (jin),\u201d a term used in Edo-period "
            "Japanese mathematics (wasan) to denote an extremely small order of magnitude. "
            "It symbolizes a world too minute to see with the naked eye, representing the "
            "project's goal of measuring molecules \u2014 the ultimate unit \u2014 one by one "
            "and deciphering the information they hold."
        )
...
class DashboardWindow(QtWidgets.QMainWindow):
    ...
    def _build_sidebar(self):
        ...
        # --- ブランドロゴ(あれば表示。なくてもアプリは問題なく動く) ---
        # ファイルの置き場所を間違えても気づけるよう、候補パスを複数探し、
        # 見つかった/見つからなかったを起動時にターミナルへ表示する。
        _logo_candidates = [
            BASE_DIR / "assets" / "jin_mark_white.png",
            BASE_DIR / "jin_mark_white.png",
        ]
        logo_path = next((p for p in _logo_candidates if p.exists()), None)
        self._logo_path = logo_path  # About JINダイアログでも使う
        if logo_path is not None:
            pix = QtGui.QPixmap(str(logo_path))
            if not pix.isNull():
                logo_lbl = _ClickableLabel()
                pix = pix.scaledToWidth(44, _Qt.SmoothTransformation)
                logo_lbl.setPixmap(pix)
                logo_lbl.setAlignment(_Qt.AlignCenter)
                logo_lbl.setCursor(QtGui.QCursor(_Qt.PointingHandCursor))
                logo_lbl.setToolTip("About JIN")
                logo_lbl.clicked.connect(self._open_about_jin)
                v.addWidget(logo_lbl)
                v.addSpacing(14)
                print(f"[info] ロゴを読み込みました: {logo_path}")
            else:
                print(f"[warn] ロゴファイルは見つかりましたが読み込めませんでした(壊れている可能性): {logo_path}")
        else:
            print(
                "[warn] ロゴファイルが見つかりません。以下のいずれかに置いてください:\n"
                + "\n".join(f"       {p}" for p in _logo_candidates)
            )

        items = [("\u2764", "Live Run", True, None), ("\u2261", "Sequence", False, None),
                 ("\u25A4", "Analysis", False, None), ("\u2913", "Export", False, None),
                 ("\u2139", "Docs", False, self._open_readme),
    ...
    def _run_modal(self, dialog):
        """ダイアログ表示中は裏で毎フレームの更新処理が動き続けて重くなる/
        ちらつくのを防ぐため、タイマーを一時停止してから開き、閉じたら
        (元々動いていた場合だけ)再開する。"""
        was_active = self.timer is not None and self.timer.isActive()
        if was_active:
            self.timer.stop()
        try:
            return dialog.exec()
    ...
    def _open_about_jin(self):
        dialog = AboutJinDialog(self, logo_path=self._logo_path)
    ...
    def _open_settings(self):
        ...
        if self._run_modal(dialog) != _DIALOG_ACCEPTED or dialog.selected_batch is None:
            return
