...
ax_gauge = fig2.add_axes([0.30, 0.685, 0.40, 0.23])
...
trend_text = fig2.text(
    0.5, 0.655, '', fontsize=FS_SUB, fontweight='bold', fontfamily=FONT_MONO,
...
warn_badge_circle = fig2.add_artist(
    mpatches.Circle((0.685, 0.895), 0.018, transform=fig2.transFigure,
                     facecolor='#FF3B30', edgecolor='white', linewidth=1.2, zorder=20)
)
warn_badge_text = fig2.text(
    0.685, 0.895, '!', fontsize=FS_LABEL, fontweight='bold', color='white',
    ha='center', va='center', zorder=21,
)
...
def _make_kpi_card(fig, x, y, w, h, label, value_color=ACCENT_COLOR2, fs_value=None):
    """角丸カード背景 + ラベル(上) + 値(下) のKPIタイルを作る。値のTextオブジェクトを返す。"""
    ...
    fig.text(x + w / 2, y + h - 0.018, label, ha='center', va='top', fontsize=FS_TICK - 1,
              fontweight='bold', fontfamily=FONT_MONO, color='#888888', zorder=11)
    value_text = fig.text(
        x + w / 2, y + h * 0.36, '', ha='center', va='center',
...
_kpi_cols = 4
_kpi_x0, _kpi_gap, _kpi_w, _kpi_h = 0.035, 0.015, 0.2175, 0.10
_kpi_row_y = [0.575, 0.46]
...
kpi_latest = _make_kpi_card(fig2, _kpi_x0 + 3 * (_kpi_w + _kpi_gap), _kpi_row_y[0], _kpi_w, _kpi_h,
                             'LATEST READ', fs_value=FS_STATUS)
...
kpi_pass = _make_kpi_card(fig2, _kpi_x0 + 2 * (_kpi_w + _kpi_gap), _kpi_row_y[1], _kpi_w, _kpi_h,
                           f'PASS RATE (\u2265Q{PASS_Q_THRESHOLD:.0f})')
kpi_conf = _make_kpi_card(fig2, _kpi_x0 + 3 * (_kpi_w + _kpi_gap), _kpi_row_y[1], _kpi_w, _kpi_h,
                           'AMBIGUOUS?', fs_value=FS_STATUS)

fig2.subplots_adjust(left=0.10, right=0.90, top=0.44, bottom=0.08)
