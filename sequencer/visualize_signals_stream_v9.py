...
# ============================
fig2 = plt.figure(figsize=(12, 15.5))
...
# 正しそうか」を、折れ線ではなく本物のクロマトグラム（Sangerシーケンサーの波形）
# のような、コードごとに色分けされたガウシアンの山として表示する。
...
for _pi in range(_ref_len):
    ...
    _glow, = ax_acc.plot(_wx, np.zeros_like(_wx), color=_wcolor, linewidth=6, alpha=0.16,
                          zorder=4, solid_capstyle='round')
    _line, = ax_acc.plot(_wx, np.zeros_like(_wx), color=_wcolor, linewidth=LW_MED, alpha=0.95,
